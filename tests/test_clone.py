...
def test_clone_vectors():
    ...
    try:
        ...
        new_file = uproot.open(new_file_name)
        new_tree = new_file[tree_name]
        assert new_tree['int_vector_branch'].array().tolist() == [[], [-1, 2]]
        assert new_tree['long_vector_branch'].array().tolist() == [[], [-3, 4]]
        assert abs(new_tree['float_vector_branch'].array() - awkward.fromiter([[], [-5.5, 6.6]])).max().max() < 1e-5
        assert abs(new_tree['double_vector_branch'].array() - awkward.fromiter([[], [-7.7, 8.8]])).max().max() < 1e-5
    finally:
        if os.path.isfile(new_file_name):
            os.remove(new_file_name)
